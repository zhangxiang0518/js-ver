...
<template>
  <div class="dBoard-item-wrapper" :id="componentsInfo.cid">
    <div class="item no-select pointer-cursor"
         :style="'height: ' + (itemH + 40) + 'px; width: ' + (itemW + 40) + 'px;box-shadow: 0 0 12px 2px '
         + dynamicBoxShadow"
         @click="handleClickItem">
      <img :src="require('../../assets/' + dynamicBgImg)" :style="'height:' + (itemH + 40) + 'px; width: ' + (itemW + 40) + 'px; filter: blur(' + dynamicBlur + 'px);'">
      ...
      <!--        Current Temp.-->
      <!--      </div>-->
            <div class="item-value" :style="'font-size: ' + Math.round((itemW)/2) + 'px; color: ' + dynamicFontColor" :data-text="humidityValue">
              <span
                  :style="{ 'font-weight': '500', 'font-family': 'Semibold, monospace'}" class="blurred-text"
              >{{ humidityValue }}</span>
<!--              <span-->
  ...
  watch: {
    componentsInfo: {
      handler: function (newval) {
        this.humidityValue = newval.value; // 首先为dom的本地温度数值变量赋值
        // if (!this.isCelsius) {
...
.item-value {
  ...
  position: absolute;
  bottom: -5%;
  left: 25%; /* this gap between value and outside wrapper is hard coded */
  font-size: 16px;
  text-align: center;
  color: #111827;
}
...
.item {
  transition: background-image 0.5s cubic-bezier(0.445, 0.050, 0.550, 0.950);
  margin: 20px auto;
...
@font-face {
  ...
  src: url('../../assets/fonts/SF Pro - Semibold.otf') format('opentype'); /* 字体文件路径 */
}

@font-face {
  font-family: 'Semibold Rails'; /* 自定义字体的名称 */
  src: url('../../assets/fonts/SF Pro - Semibold Rails.otf') format('opentype'); /* 字体文件路径 */
}

@font-face {
  font-family: 'Semibold Rounded'; /* 自定义字体的名称 */
  src: url('../../assets/fonts/SF Pro - Semibold Rounded.otf') format('opentype'); /* 字体文件路径 */
}

@font-face {
  font-family: 'Semibold Soft'; /* 自定义字体的名称 */
  src: url('../../assets/fonts/SF Pro - Semibold Soft.otf') format('opentype'); /* 字体文件路径 */
}

@font-face {
  font-family: 'Semibold Stencil'; /* 自定义字体的名称 */
  src: url('../../assets/fonts/SF Pro - Semibold Stencil.otf') format('opentype'); /* 字体文件路径 */
}
</style>
<style></style>
